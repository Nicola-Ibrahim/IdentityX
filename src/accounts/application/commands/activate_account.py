...
class ActivateAccountCommand(BaseCommand[AuthDTO], BaseModel):
    account_id: str


class ActivateAccountHandler(BaseCommandHandler[ActivateAccountCommand, AuthDTO]):
    def __init__(self, account_repo: BaseAccountRepository):
        self._account_repo = account_repo

    async def handle(self, command: ActivateAccountCommand) -> AuthDTO:
        try:
