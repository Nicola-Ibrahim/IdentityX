...
class SQLBaseAccountRepository(BaseAccountRepository):
    ...
    def _to_domain(self, record: AccountORM) -> Account:
        ...
        session_ids = []
        try:
            session_ids = [SessionId.create(uuid.UUID(s.session_uuid)) for s in record.sessions]
        except Exception:
    ...
    async def update(self, account: Account) -> None:
        result = await self._session.execute(select(AccountORM).filter(AccountORM.uuid == str(account.id.value)))
    ...
    async def get_by_id(self, account_id: AccountId) -> Account | None:
        stmt = (
            select(AccountORM)
            .options(selectinload(AccountORM.sessions))
            .filter(AccountORM.uuid == str(account_id.value))
        )
    ...
    async def exists_by_email(self, email: str) -> bool:
        result = await self._session.execute(select(AccountORM.uuid).filter(AccountORM.email == email))
        return result.first() is not None

    async def list_accounts(self) -> Iterable[Account]:
        result = await self._session.execute(select(AccountORM).order_by(AccountORM.created_at.asc()))
        records = result.scalars().all()
        return [self._to_domain(record) for record in records]

    async def remove(self, account_id: AccountId) -> None:
        result = await self._session.execute(select(AccountORM).filter(AccountORM.uuid == str(account_id.value)))
        record = result.scalars().first()
        if record:
